...
import { getAllAcademicYears } from "../../../../services/academicYearAPI";
...
const Attendance = ({ toggleSidebar }) => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        setLoading(true);
        const currentUser = JSON.parse(localStorage.getItem('grace_erp_user')) || {};
        const adminStatus = currentUser.role === 'admin' || currentUser.role === 'principle' || currentUser.role === 'HR';
        setIsAdmin(adminStatus);

        const [_years, classRes, currentYear] = await Promise.all([
          getAllAcademicYears(),
          adminStatus ? getClassrooms() : getTeacherProfile(),
          getActiveYear()
        ]);
    ...
    <div className="p-4 lg:p-8 bg-bg-base min-h-screen animate-in fade-in duration-500 font-sans text-left">
      ...
      >
        <div className="flex flex-nowrap items-center gap-4">
          <div className="flex items-center bg-white/5 border border-white/10 px-5 py-3 rounded-2xl shadow-xl group/cls hover:border-amber-400/30 transition-all">
          ...
          <tr key={s.student_id} className="hover:bg-slate-50/50 transition-colors group border-b border-black/10 last:border-b-0">
            ...
            <td className="px-8 py-6 border-r border-b border-black">
              <div className="flex items-center gap-4">
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center font-bold text-black text-sm border border-black shadow-sm transition-all duration-500 group-hover:scale-110 ${s.status === 'present' ? 'bg-emerald-500' :
                ...
                <div>
                  ...
                  <p className="text-[9px] font-bold text-black uppercase tracking-widest mt-2">ID: {s.id_no || s.student_id_no || 'NGA-00' + s.student_id}</p>
                </div>
              </div>
            </td>
            <td className="px-8 py-6 border-r border-b border-black">
  ...
  return (
    ...
    >
      <div className="flex-1">
        <p className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1.5 opacity-60 ${theme.text}`}>{label}</p>
        <h3 className={`text-4xl font-black tracking-tighter leading-none ${theme.text}`}>{value || 0}</h3>

...
const HROverview = ({ toggleSidebar }) => {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            // Fetch staff stats
            const staffRes = await axios.get('http://localhost:5000/api/staff/stats');
            
            // Fetch all staff for recent additions
            const allStaffRes = await axios.get('http://localhost:5000/api/staff/all');
            ...
            try {
                const attRes = await axios.get(`http://localhost:5000/api/attendance/staff?date=${today}`, { headers });
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-6 group hover:shadow-md transition-all">
                    ...
                    <div>
                        ...
                        <p className="text-3xl font-black text-[#001736]">{stats.onLeaveToday}</p>
                    </div>
                </div>
            </div>
        </div>

...
const FeeStructureManager = ({ selectedYear, selectedYearName, isMobileSearchOpen }) => {
    ...
    const handleDeleteRecord = async (id) => {
        if (!window.confirm("Are you sure you want to permanently remove this fee structure? This cannot be undone.")) return;
        try {
            setSaving(true);
    ...
    const handleExportPDF = () => {
        const doc = new jsPDF();
        const schoolName = "GRACE ENGLISH SCHOOL";
    ...
    const filteredRegistry = useMemo(() => {
        return registry
            ...
            .filter(r => r.grade.toLowerCase().includes(searchQuery.toLowerCase()));
    }, [registry, selectedClassFilter, searchQuery]);
        ...
        <div className="p-0 md:p-0 space-y-12 animate-in fade-in duration-500 min-h-screen font-sans">
            ...
            {viewMode === 'registry' && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                        ...
                        return (
                            <div key={card.range} className={`p-6 rounded-2xl border-l-4 transition-all duration-300 group hover:shadow-xl flex items-center justify-between gap-5 shadow-sm ${theme.border} ${theme.bg}`}>
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <button onClick={() => setViewMode('transportForm')} className="p-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity bg-white/80 rounded-lg hover:bg-white text-slate-400 hover:text-[#001736] border border-black/5 flex items-center justify-center cursor-pointer">
            ...
            {viewMode === 'registry' ? (
                <>
                    ...
                    <div className={`${isMobileSearchOpen ? 'flex' : 'hidden lg:flex'} flex-col lg:flex-row items-center justify-between gap-6 p-6 lg:p-0 bg-white lg:bg-transparent rounded-3xl lg:rounded-none shadow-2xl lg:shadow-none animate-in slide-in-from-top-2 duration-300`}>
                        <div className="flex flex-1 items-center gap-4 w-full md:max-w-xl">
                            <div className="relative flex-1 group">
                                <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 group-focus-within:text-[#001736] transition-colors" />
                                <input
                                    type="text" placeholder="SEARCH REGISTRY..." value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    className="w-full pl-14 pr-8 py-4 bg-white border border-institutional rounded-2xl text-[11px] font-black uppercase tracking-widest outline-none shadow-sm"
                                />
                            </div>
                            <div className="hidden sm:flex items-center gap-3 bg-white px-6 py-0 rounded-xl border border-institutional shadow-sm focus-within:ring-4 focus-within:ring-[#001736]/5">
                                <Filter className="w-4 h-4 text-slate-400 ml-4" />
                                <select
                                    value={selectedClassFilter}
                                    onChange={(e) => setSelectedClassFilter(e.target.value)}
                                    className="h-12 bg-transparent text-[10px] font-black text-[#001736] uppercase tracking-widest outline-none pr-6 cursor-pointer"
                                >
                        ...
                        <div className="flex items-center gap-4 w-full lg:w-auto">
                            <button onClick={handleExportPDF} className="flex-1 lg:flex-none p-4 rounded-xl border border-institutional text-[#001736] hover:bg-slate-50 transition-all flex items-center justify-center gap-3 cursor-pointer">
                                <Download className="w-5 h-5" />
                                <span className="text-[10px] font-black uppercase tracking-widest">Download</span>
                            </button>
                            <button onClick={() => setViewMode('transportForm')} className="flex-1 lg:flex-none p-4 rounded-xl border border-institutional btn-add-institutional transition-all flex items-center justify-center gap-3 cursor-pointer">
                            ...
                            <button
                                ...
                                className="flex-2 lg:flex-none btn-add-institutional px-8 py-4 rounded-xl font-black text-[11px] uppercase tracking-[0.3em] shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-4 cursor-pointer"
                            >
                            ...
                            <tr key={idx} className="hover-table-row transition-colors group">
                                <td className="px-6 py-4 border-b-table border-r-table">
                                    <div className="flex items-center gap-5 text-left">
                                        <div className="w-10 h-10 bg-white border-table rounded-xl flex items-center justify-center text-sm font-black text-[#001736] shadow-sm">{row.grade.charAt(0)}</div>
                                        <div>
                                            <p className="text-[12px] font-bold text-table-cell uppercase tracking-tight">{row.grade}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td className="px-6 py-4 text-center border-b-table">
                                    <div className="flex justify-center gap-3">
                                        <button onClick={() => setViewData(row)} className="p-2.5 bg-white border-table text-table-cell rounded-xl cursor-pointer hover:bg-gray-400 hover:text-white transition-all shadow-sm active:scale-95" title="View Details"><Eye className="w-3.5 h-3.5" /></button>
                                        <button onClick={() => {
                                            setEditingGrade({ gradeName: row.grade, studentType: row.studentType, heads: row.heads, transport: row.transport });
                                            setViewMode('classForm');
                                        }} className="p-2.5 bg-white border-table text-table-cell rounded-xl cursor-pointer hover:bg-gray-400 hover:text-white transition-all shadow-sm active:scale-95" title="Edit Structure"><Edit3 className="w-3.5 h-3.5" /></button>
                                        <button onClick={() => handleDeleteRecord(row.id)} className="p-2.5 bg-white border-table text-table-cell rounded-xl cursor-pointer hover:bg-gray-400 hover:text-white transition-all shadow-sm active:scale-95" title="Delete Record"><Trash2 className="w-3.5 h-3.5" /></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
                ...
                <div className="animate-in slide-in-from-bottom-8 duration-700 bg-white rounded-xl border border-institutional shadow-3xl p-10 lg:p-14 space-y-12">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-8 border-b border-slate-100 pb-10">
                        <div className="flex items-center gap-8 text-left">
                            <button onClick={() => setViewMode('registry')} className="p-4 rounded-2xl border border-institutional text-slate-400 hover:bg-[#001736] hover:text-white transition-all cursor-pointer">
                        ...
                        <div className="flex items-center gap-6">
                            <div className="p-1.5 rounded-xl flex border border-institutional">
                                {['new', 'old'].map(type => (
                                    <button
                                        key={type} onClick={() => setEditingGrade(prev => ({ ...prev, studentType: type }))}
                                        className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${editingGrade.studentType === type ? 'bg-amber-400 text-white shadow-lg shadow-amber-100' : 'text-slate-400'} cursor-pointer`}
                                    >
                                        {type}
                                    </button>
                                ))}
                            ...
                            <div className="relative group">
                                <select
                                    ...
                                    className="w-full border border-institutional px-8 py-4 rounded-2xl font-bold text-[11px] uppercase outline-none shadow-sm min-w-48 appearance-none bg-white cursor-pointer"
                                >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        {[
                            ...
                            <div key={box.label} className="bg-slate-50 p-6 rounded-2xl border border-institutional text-center space-y-2">
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{box.label}</p>
                                <h4 className={`text-3xl font-black text-${box.color}-600`}>₹{box.val.toLocaleString()}</h4>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 px-4 text-left">
                        {[
                            ...
                            <div key={field.id} className="space-y-2">
                                ...
                                <div className="flex items-center gap-4 bg-white border-institutional border p-4 rounded-xl focus-within:ring-4 focus-within:ring-[#001736]/5 transition-all">
                    ...
                    <div className="flex flex-col items-center pt-10">
                        <button
                            ...
                            className="w-full md:w-auto min-w-96 py-6 bg-[#001736] text-white rounded-xl text-[12px] font-black uppercase tracking-[0.5em] shadow-2xl hover:bg-black transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
                        >
                ...
                <div className="animate-in slide-in-from-bottom-8 duration-700 bg-white rounded-xl border border-institutional shadow-3xl p-10 lg:p-14 space-y-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-left">
                        {[
                            ...
                            <div key={range.id} className="space-y-4">
                                ...
                                <div className="mt-3 flex items-center gap-4 bg-slate-50 border border-institutional p-3 rounded-2xl focus-within:bg-white focus-within:ring-8 focus-within:ring-[#001736]/5 transition-all">
                    ...
                    <div className="flex flex-col items-center pt-10">
                        <button
                            ...
                            className="w-full md:w-auto min-w-96 py-6 bg-[#001736] text-white  rounded-xl text-[12px] font-black uppercase tracking-[0.5em] shadow-2xl hover:bg-[#001736] transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
                        >
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-100 flex items-center justify-center p-6 animate-in fade-in duration-300">
                    <div className="bg-white rounded-3xl w-full max-w-2xl shadow-4xl overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="bg-[#001736] p-8 flex items-center justify-between text-white">
                            <div>
                                <h3 className="text-2xl font-black uppercase tracking-tight">{viewData.grade} Class Fee Details</h3>
                                <p className="text-[10px] uppercase font-bold tracking-widest opacity-60">Term-wise distribution breakdown</p>
                            </div>
                            <button onClick={() => setViewData(null)} className="p-3 hover:bg-white/10 rounded-xl transition-all cursor-pointer">
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                        <div className="p-10 space-y-8">
                            <div className="grid grid-cols-3 gap-8 text-left">
                                <div className="space-y-4 p-6 bg-slate-50 rounded-2xl border border-institutional">

...
const LibraryProfile = ({ toggleSidebar }) => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                // Librarian is a staff member, fetch from staff records
                const data = await getStaffProfile(userData.id);
                setProfile(data);
            } catch (error) {
                console.error("Fetch Profile Error:", error);
                toast.error("Failed to load librarian profile. Verify your HR registration.");
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
            <div className="w-10 h-10 border-4 border-[#001736]/20 border-t-[#001736] rounded-full animate-spin"></div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Scanning Identity Profile...</p>
        </div>
    );

    return (
        <div className="p-4 sm:p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 font-sans text-left bg-[#F8FAFC]">
            <ModuleHeader

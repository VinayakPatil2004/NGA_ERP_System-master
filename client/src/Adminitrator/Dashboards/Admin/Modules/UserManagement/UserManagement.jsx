...
const UserManagement = ({ toggleSidebar }) => {
    ...
    useEffect(() => {
        ...
        if (userTypeFilter !== 'all') {
            if (userTypeFilter === 'staff') {
                // Include all internal personnel roles
                result = result.filter(u => !['student', 'parent', 'admin'].includes(u.role_name?.toLowerCase()));
            } else if (userTypeFilter === 'student') {
                result = result.filter(u => u.role_name?.toLowerCase() === 'student' || u.role_name?.toLowerCase() === 'alumni');
            } else if (userTypeFilter === 'parent') {
                result = result.filter(u => u.role_name?.toLowerCase() === 'parent' || u.role_name?.toLowerCase() === 'alumni parent');
            }
        }
    ...
    const handleToggleBlock = async (user) => {
        ...
        if (window.confirm(`Permanently ${action} institutional access for ${user.username || 'this identity'}?`)) {
            try {
                await toggleBlockUser(user.id, !user.is_blocked, user.type);

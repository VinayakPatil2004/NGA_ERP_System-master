...
const Reports = ({ toggleSidebar }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const [caData, expenseData] = await Promise.all([
                    financeAPI.getCAReports(),
    ...
    // Combine for the Cash Flow Matrix visualization
    const transactions = [
        ...expenses.map(e => ({
            id: e.id,
            title: e.description,
            desc: e.category,
            date: new Date(e.expense_date).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' }).toUpperCase(),
            amount: `- ₹ ${e.amount}`,
            type: 'expense'
        })),
        ...reports.map(r => ({
            ...
            title: r.title,
            desc: "CA Audit Entry",
            date: new Date(r.report_date).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' }).toUpperCase(),
            amount: r.financial_impact || "N/A",
            type: 'income' // Assumption: CA reports highlight captured/verified income or impact
        }))
                ...
                <div className="space-y-3">
                    {transactions.length > 0 ? (
                        ...
                        <div className="p-12 text-center bg-slate-50 border border-dashed border-slate-200 rounded-3xl">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Registry Zero: No Recorded Transactions</p>
                        </div>
                    )}
                </div>
            </div>

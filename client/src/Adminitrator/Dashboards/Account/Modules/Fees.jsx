...
const Fees = ({ toggleSidebar }) => {
    ...
    useEffect(() => {
        const fetchLedger = async () => {
            try {
                setLoading(true);
                const data = await financeAPI.getStudentFeeLedger();
                // Map backend keys (e.g., student_name) to frontend expectations
                const mappedData = data.map(s => ({
                    srNo: s.student_id_no || s.id,
                    name: s.student_name,
                    grade: s.grade,
                    totalFees: s.total_payable || 0,
                    paid: s.total_paid || 0,
                    pending: (s.total_payable || 0) - (s.total_paid || 0),
                    lastPayment: null, // Not explicitly in the first 50 lines of query
                    phone: s.father_mobile,
                    parent: s.father_name
                }));
                setStudentsData(mappedData);
            } catch (error) {
                console.error("Failed to fetch fee ledger:", error);
